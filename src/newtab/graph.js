...
export default class Graph {
	constructor() {
		...
		this.y_scale = d3.scaleLinear().range([svg_height, 0])

		// this will set the coordinates of a line's data points according
		// to the corresponding values
		this.line = d3.line()
			.x((function(d) { return this.x_scale(d.timestamp) }).bind(this))
			.y((function(d) { return this.y_scale(d.value) }).bind(this))

		this.bisectDate = d3.bisector(d => { return d.timestamp }).left;

		// set viewBox so that the line gets stretched to whole screen without
		// updating the data points of the line
		this.graph_svg = d3.select('#graph')
		...
		this.date_lbl = this.legend.append('text')
			.attr('y', 18)
			.attr('x', 10)

		this.trend_lbl = this.legend.append('text')
			.attr('y', 51)
			.attr('x', 30)

		this.cycle_lbl = this.legend.append('text')
			.attr('y', 79)
			.attr('x', 30)
		...
		for (var n in this.lines) {
			this.lines[n].line = this.graph_g.append('path')
			...
			this.lines[n].current = this.overlay_g.append('circle')
				.attr('r', 6)
	...
	setData(data) {
		...
		var time_domain = d3.extent(this.data.map(function(d) { return d.timestamp }))
		...
		var value_domain = [
			...
			d3.min(this.data.map(function(d) { 
				return d3.min(Object.keys(lines).map(function(name) { return d[name] }))
			})),
			// obtain maximum of data points that will be plotted
			d3.max(this.data.map(function(d) { 
				return d3.max(Object.keys(lines).map(function(name) { return d[name] }))
			}))
		]
		...
		for (var n in this.lines) {
			var line_data = this.data.map(function(d) { return { timestamp: d.timestamp, value: d[n] } })
	...
	onMousemove() {
		...
		for (var n in this.lines) {
			this.lines[n].current.attr('cx', dx)
				.attr('cy', this.y_scale(d[n]) * window.innerHeight / svg_height);
		}
